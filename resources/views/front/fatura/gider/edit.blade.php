        ...
        <div class="col-md-12">
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">Gider Kayıt Ekranı</h3>
                ...
                <form method="post" enctype="multipart/form-data" action="{{route("fatura.update",$data[0]["id"])}}">
                    <div class="row p-3">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="inputClientCompany">Fatura No</label>
                                <input type="text" value="{{$data[0]["faturaNo"]}}" name="faturaNo" required id="adres"
                                    class="form-control">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="date" name="faturaTarih" required id="faturaTarihi"
                                    value="{{$data[0]["faturaTarih"]}}" class="form-control">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select name="musteriId" required class="form-control select2 select2-danger"
                                    ...
                                    <option value="{{$musteri->id}}" @if($musteri["id"]==$data[0]["musteriId"]) selected
                                        @endif>{{$musteri->ad}} {{$musteri->soyad}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <table class="table" id="faturaData">
                        ...
                        <tbody>
                            @foreach ($dataIslem as $itemKey => $itemVal)
                            <tr class="islem_field">
                                <td><select name="islem[{{$itemKey}}[kalemId]" class="form-control kalem"
                                        value="islem[{{$itemKey}}][kalemId]">
                                        <option value="0">Kalem Seçiniz</option>
                                        @foreach($kalemGider as $item)
                                        <option @if($itemVal["kalemId"]==$item->id) selected @endif
                                            data-k="{{$item["kdv"]}}"
                                            value="{{$item["id"]}}">
                                            {{$item["ad"]}}</option>
                                        @endforeach
                                ...
                                <td><select name="islem[{{$itemKey}}][urun_id]" class="form-control urun"
                                        value="islem[{{$itemKey}}][urun_id]">
                                        <option value="0">Ürün Seçiniz</option>
                                        @foreach($urunler as $item)
                                        <option @if($itemVal["urun_id"]==$item->id) selected @endif
                                            data-fiyat="{{$item["satis_fiyati"]}}"
                                            value="{{$item["id"]}}">
                                            {{$item["urun_adi"]}}</option>
                                        @endforeach
                                    </select></td>
                                <td><input type="text" class="form-control" id="gun_adet"
                                        name="islem[{{$itemKey}}][gun_adet]" value="{{$itemVal["miktar"]}}" />

                                <td><input type="text" class="form-control" id="tutar" name="islem[{{$itemKey}}][tutar]"
                                        value="{{$itemVal["fiyat"]}}" />

                                <td><input type="text" class="form-control" id="toplam_tutar"
                                        name="islem[{{$itemKey}}][toplam_tutar]" value="{{$itemVal["araToplam"]}}" />

                                <td><input type="text" class="form-control" id="kdv" value="{{$itemVal["kdv"]}}"
                                        name="islem[{{$itemKey}}][kdv]" />

                                <td><input type="text" class="form-control" value="{{$itemVal["kdvToplam"]}}"
                                        id="kdv_toplam" name="islem[{{$itemKey}}][kdv_toplam]" />

                                <td><input type="text" class="form-control" value="{{$itemVal["genelToplam"]}}"
                                        id="genel_toplam" name="islem[{{$itemKey}}][genel_toplam]" />

                                <td><input type="text" class="form-control" value="{{$itemVal["text"]}}" id="text"
                                        name="islem[{{$itemKey}}][text]" />
...
<script>
    ...
    $("#addRowBtn").click(function(){
        ...
        '<option value="0">Ürün Seçiniz</option>';
        @foreach($urunler as $item)
        newRow += '<option data-fiyat="{{$item["alis_fiyati"]}}" value="{{$item["id"]}}">{{$item["urun_adi"]}}</option>';
        @endforeach
            newRow += "</select></td>" +
